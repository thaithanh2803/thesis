...
############ 1. IS Regressions
  ...
  IS_fit=jma(y=tmp_data[,1],x=tmp_data[,-1],
             ma.method="MMA",#c("JMA","MMA")
             model.subset="all")#c("nested","all"),
...
############# 2. OOS Regressions
  ...
  for(i in loop_index){
    ...
    OOS_fit=jma(y=train_data[,1],x=train_data[,-1],
                ma.method="MMA",
                model.subset="all")#c("nested","all"),
...
############### Computing statistics
  ...
  return(list(SSE=SSE,
              statistics_table=data.frame(Algorithm="mma",
...
```{r}
find_cv=function(data,feature,forecast_start=20,B=499,cpucore=2){
########### Data preparation
...
########## bootstrap
    ...
    metric=foreach(icount(B), .combine='rbind',.packages=c("xts","lubridate","dplyr","caret","MAMI"),.export = c("statistics")) %dopar% {
...
if (!dir.exists(mmaDir)) {
  cat(paste0("mma Output folder does not exist. Please check the working direction"))
} else {
      ### Compute all statistics
    a=mma_fct(dat,feature=c("b.m","logDY","logEP","ntis","tbl"),forecast_start=1965)
      ### Diagnostic plot
    png(filename = paste0(mmaDir,"/mma.png"))
    plot_figure(a[[1]],ylim=NULL,main = "MMA")
    dev.off()
      ### Compute critical values
    critical_value=find_cv(dat,feature=c("b.m","logDY","logEP","ntis","tbl"),forecast_start=1965,B=999,cpucore = 4)
  }

```

```{r}
write.csv(a[[2]],paste0(mmaDir,"/mma_all_statistics.csv"),row.names = FALSE)
write.csv(critical_value,paste0(mmaDir,"/mma_critical_value.csv"))
write.zoo(a[[1]],sep=",",paste0(mmaDir,"/mma_SSE.csv"))
```
