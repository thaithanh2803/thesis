...
pcannDir=file.path(mainDir,"pcann")
dir.create(pcannDir)
```
...
############# Data preparation
  ...
  preObj=preProcess(tmp_data[,-1,drop=FALSE],method = c("center","scale","pca"),pcaComp = pcaComp)
  tmp_x=predict(preObj,tmp_data[,-1,drop=FALSE]) %>% as.matrix()
  tmp_y=as.matrix(tmp_data[,1,drop=FALSE])
...
############# 2. OOS Regressions
  ...
  for(i in loop_index){
    ...
    preObj=preProcess(train_data[,-1,drop=FALSE],method = c("center","scale","pca"),pcaComp = pcaComp)
    train_x=predict(preObj,train_data[,-1,drop=FALSE]) %>% as.matrix()
    train_y=as.matrix(train_data[,1,drop=FALSE])
      ### Forecast data
    forecast_data=tmp[i+months(1)] %>% as.data.frame()
    forecast_x=predict(preObj,forecast_data[,-1,drop=FALSE]) %>% as.matrix()
    forecast_y=as.matrix(forecast_data[,1,drop=FALSE])
      ### OOS data 
  ...
  foreach(i=1:3, .combine='c') %do% {
    ...
    rm(OOS_fit,cv_errors,best_neurons,cv_mse,preObj)
    gc(verbose = FALSE)
    k_clear_session() 
...
############### Computing statistics
  ...
  return(list(SSE=SSE,
              statistics_table=data.frame(Algorithm="pcann",
...
statistics=function(data,forecast_start=20,pcaComp=1){
...
############ 1. IS Regressions
  ...
  rm(IS_fit,preObj,tmp_data,tmp_x,tmp_y)
  gc(verbose = FALSE)
  k_clear_session()
...
```{r}
find_cv=function(data,forecast_start=20,B=499,pcaComp = 1){
########### Data preparation
...
########## bootstrap
  metric=foreach(icount(B), .combine='rbind') %do% {
        ...
        a=statistics(data.boot,forecast_start=forecast_start,pcaComp = pcaComp) 
        return(a)
  }
...
if (!dir.exists(pcannDir)) {
  cat(paste0("pcann Output folder does not exist. Please check the working direction"))
} else {
      ### Compute all statistics
    a=pcann(dat,forecast_start=1965,pcaComp = 1)
      ### Diagnostic plot
    png(filename = paste0(pcannDir,"/pcann.png"))
    plot_figure(a[[1]],ylim=NULL,main = "PCANN")
    dev.off()
      ### Compute critical values
    critical_value=find_cv(dat,forecast_start=1965,B=999,pcaComp = 1)
  }

```

```{r}
write.csv(a[[2]],paste0(pcannDir,"/pcann_all_statistics.csv"),row.names = FALSE)
write.csv(critical_value,paste0(pcannDir,"/pcann_critical_value.csv"))
write.zoo(a[[1]],sep=",",paste0(pcannDir,"/pcann_SSE.csv"))
```
